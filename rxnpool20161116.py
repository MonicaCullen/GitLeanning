...
def Testsmi(sm='',rxn =''):
    ...
    if rxn:
        ...
        if rule:
            ...
            print "pattern error"
def standSmidictFile(SmidictFile):

        SmidictFile = json.load(open('./ChEBISmi_NameId.json'))
        
        standSmiNameId = dict()
        
        for smi,nameId in ChEBISmiNameId.items():
            if smi == '[H]O[H]':
                smi = 'O'
                ###modified
                standSmi = NeutraliseCharges(smi)[0]
                if not standSmi:
                    standSmi = smi #如果标准化不成功，则还是用原来的smiles
                standSmiNameId[standSmi] = nameId
            else:
                continue
        with open('./StandChEBISmi_NameId.json','w') as f1:
            json.dump(standSmiNameId,f1,indent = 2)
...
def smirks2rxnName(smirks): 
    ...
    try:
        StandSmiNameId = json.load(open('./StandChEBISmi_NameId.json'))
    except:
        ChEBISmiNameId = json.load(open('./ChEBISmi_NameId.json'))
    ...
    _rs = list(set(rs))
    _ps = list(set(ps))
    coefRs = list();coefRsName = list()
    coefPs = list();coefPsName = list()
    ...
    for m,it in enumerate([_rs,_ps]):
        ...
        for s in it:
            ...
            if num != 1:
                index1[str(m)].append(str(num) + ' ' + s)
                index2[str(m)].append(str(num) + ' ' + str(sName))
            else:
                index1[str(m)].append(s)
